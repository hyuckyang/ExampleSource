...
void URobotActAnimAttackNotifyState::NotifyTick(USkeletalMeshComponent * MeshComp, UAnimSequenceBase * Animation, float FrameDeltaTime)
{
	...
	if (coreAct->m_Sight == nullptr) return;

	// 아래 크래쉬 원인 찾는 중
	return;

	if (coreAct->m_Sight->IsAttackCharacter() == false) return;

	TArray<ACoreActCharacter*> attackAct = coreAct->m_Sight->GetPossibleAttackActList();
